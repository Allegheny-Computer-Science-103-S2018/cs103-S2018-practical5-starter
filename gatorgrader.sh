...
if [ "$CHECK" = true ]; then
  ...
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/www \
                                     --checkfiles index.html --fragments "<title>My Travel Checklist</title>" --fragmentcounts 1
  determine_exit_code $?
  # --> GatorGrader CHECK:
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/www \
                                     --checkfiles index.html --fragments "<h1>My Travel Checklist</h1>" --fragmentcounts 1
  determine_exit_code $?
  # --> GatorGrader CHECK:
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/www \
